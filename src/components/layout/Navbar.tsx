...
const Navbar = ({ variant = 'transparent', className = '' }: NavbarProps) => {
    ...
    if (!mounted) {
        return (
            <nav className="fixed top-0 left-0 right-0 z-50 h-14 sm:h-16 md:h-18 lg:h-20 bg-transparent">
    ...
    return (
        <>
                ...
                <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 h-full">
                    <div className="flex justify-between items-center h-full">
                        ...
                        <div className="hidden lg:flex lg:items-center lg:space-x-4 xl:space-x-8">
                            {navLinks.map((link) => (
                                <Link
                                    ...
                                    href={link.href}
                                    className={`
                                        relative px-2 py-1.5 text-sm font-medium lg:px-3 lg:py-2 
                        ...
                        <div className="lg:hidden">
                            <button
                                onClick={toggleMenu}
                                className="p-2.5 rounded-lg text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors duration-200 min-h-11 min-w-11 flex items-center justify-center"
            ...
            <div className={`
                fixed top-0 right-0 z-50 h-full w-72 max-w-[85vw] sm:w-80 sm:max-w-sm 
                bg-white dark:bg-neutral-900 shadow-xl
                transform transition-transform duration-300 ease-in-out lg:hidden
                ${isOpen ? 'translate-x-0' : 'translate-x-full'}
            `}>
                {/* Header del menú */}
                <div className="flex items-center justify-end p-4 sm:p-5 border-b border-neutral-200 dark:border-neutral-700">
                    <button
                        onClick={closeMenu}
                        className="p-2.5 rounded-lg text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-neutral-100 dark:hover:bg-neutral-800 min-h-11 min-w-11 flex items-center justify-center"
                ...
                <div className="p-4 sm:p-5 space-y-3">
                    {navLinks.map((link) => (
                        <Link
                            ...
                            onClick={closeMenu}
                            className={`
                                block px-4 py-4 rounded-lg text-lg font-medium sm:text-base sm:py-3 min-h-12
                ...
                {/* Footer del menú */}
                <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 border-t border-neutral-200 dark:border-neutral-700">
